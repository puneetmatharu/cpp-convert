...
def main(argv):
    ...
    if sys.version_info < (3, 6):
        print("You need at least Python 3.6 ", file=sys.stderr)
        sys.exit(3)

    major = sys.version_info[0]
    if major > 3:
        print("Python 4 and above versions are not supported yet ", file=sys.stderr)
        sys.exit(3)
    ...
    if ok_to_write_ofile(overwrite_flag, ifilename, ofilename):
        ...
        print(f"* Conversion done in {end - start:.5f} seconds")
        print(f'* Output file name: {ofilename} ')
...
def tecplot_to_vtkxml(inputFilename, outputFilename):
    ...
    for fieldIndex in range(fieldsCount):
        sys.stdout.write("Write field %02d/%02d........................." %
                         (fieldIndex + 1, fieldsCount))
        sys.stdout.flush()
...
class TecplotZone:
    ...
    def parse(file, line):
        ...
        for nodeIndex in range(nodesCount):
            ...
            if len(data) < dim:
                raise TecplotParsingError(
                    "Invalid zone", "not enough values for this node ! Try to convert with -p option", line)
            ...
            for i, value in enumerate(data):
                try:
                    ...
                    raise TecplotParsingError(
                        "Invalid zone", "wrong node values ! Try to convert with -p option", line)
        ...
        if format == 1:
            for cellIndex in range(cellsCount):
                ...
                data = data.strip().split(" ")
                if cell == 3:
                    cellNodes = 2  # VTK_LINE
                if cell == 5:
                    cellNodes = 3  # VTK_TRIANGLE
                if cell == 9:
                    cellNodes = 4  # VTK_QUAD
                if cell == 10:
                    cellNodes = 4  # VTK_TETRAHEDRON
                if cell == 12:
                    cellNodes = 8  # VTK_HEXAHEDRON

                if len(data) != cellNodes:
                    raise TecplotParsingError(
                        "Invalid zone", "wrong connectivity list for this element!", line)
